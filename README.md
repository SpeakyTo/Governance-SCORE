...
Here is a checklist you will need to follow in order to deploy SpeakyTo to the Yeouido testnet:

  * Install prerequisites:
    * `python3 -m venv ./venv && source ./venv/bin/activate`
    * `pip install tbears`
    * `sudo apt install jq`
  * Clone the SpeakyTo repository:
    * `git clone https://github.com/iconation/SpeakyTo.git && cd SpeakyTo`
  * Start tbears using the `start_tbears.sh` script located at the root folder of the SpeakyTo repository
    * `./start_tbears.sh`
...
## Deploy SpeakyTo SCORE to localhost, testnet or mainnet
...
## Update an already deployed SpeakyTo to localhost, testnet or mainnet

- If you modified the SpeakyTo SCORE source code, you may need to update it.
